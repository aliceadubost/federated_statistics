    ...
    showModal(modalDialog(
      ...
      if (nzchar(onboarding_msg)) tags$details(
        ...
        div(style = "margin-top:10px;",
            ...
            div(class = "sec-lbl", "Site kit"),
            p(class = "note", style = "margin-top:0;",
              "Only works once the operator is on the tailnet — clicking it before that ",
              "will just fail to load, that's expected."),
            div(
              tags$a("Download kit", href = kit_link, target = "_blank", title = kit_link,
                    class = "btn btn-primary btn-sm", style = "margin-right:6px;"),
              tags$input(id = "kit_link_input", class = "form-control", readonly = NA,
                        value = kit_link, style = "display:none;"),
