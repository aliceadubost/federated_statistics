...
- **Sensitive-query budget and audit log.** The site keeps a persistent query audit log (`engine/site/site_query_audit.jsonl`) and rate-limits repeated sensitive probes. Exact duplicate requests for descriptive/validation endpoints are blocked briefly, and too many distinct sensitive probes in one window are refused, to make differencing-style exploration harder.
- **Validation output is deliberately sparse.** `validate_data()` reports enough to catch missing columns, bad coding, missingness and gross range problems, but does not expose exact minima/maxima, tiny outlier counts, or rare category counts. Quartiles are held back until a stricter sample-size gate is met.
...
- **Operational guardrails.** By default the coordinator requires at least two registered sites before validation or analysis (`FED_MIN_ACTIVE_SITES`), and it keeps a persistent study audit log (`engine/coordinator/study_log.jsonl`) of validation and analysis runs, including privacy-suppression warnings.
